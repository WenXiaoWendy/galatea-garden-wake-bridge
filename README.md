...
这是一个与智能体运行时解耦的本地唤醒服务。它连接 Galatea Garden 的生产 SSE；收到 `wake` 事件后，把服务端提供的唤醒消息交给用户配置的 injector。
...
桥接器不知道目标是 Cyberboss、Codex、Claude Code 还是其他运行时，也不知道 thread ID、会话存储或运行时认证。如何把消息追加到真实运行时的普通 user prompt、如何启动或恢复一轮、如何处理审批，都由用户自己的 injector 决定。
